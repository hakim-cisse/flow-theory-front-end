...
const capabilityKeys = ["clickup", "agents", "workflows", "integrations", "reports", "governance", "data", "andmore"] as const;
...
const AIAgentsPage = () => {
  ...
  return (
    <SiteLayout>
      <SEO
        title="ClickUp Integration & AI Agents"
        description="We connect ClickUp to your stack and deploy autonomous AI agents that update tasks, draft status updates, and move work forward inside the tools your team already uses."
        canonicalUrl="https://www.flowtheoryai.com/services/ai-agents"
      />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-end">
              <div className="lg:col-span-7" style={staggerStyle(0, heroVisible)}>
                <span className="text-mono text-primary/70 block mb-6">
                  {t("svc.agents.eyebrow")}
                </span>
                <h1 className="text-display font-serif">{t("svc.agents.title")}</h1>
                <div className="accent-bar mt-6" />
              </div>
              <div className="lg:col-span-5 space-y-5" style={staggerStyle(1, heroVisible)}>
                <p className="text-lead text-foreground/90">{t("svc.agents.lead1")}</p>
                <p className="text-body text-muted-foreground">{t("svc.agents.lead2")}</p>
        ...
        <div ref={capRef} className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-screen-2xl mx-auto">
            <div className="mb-12 max-w-4xl" style={staggerStyle(0, capVisible)}>
              <span className="text-mono text-primary/70 block mb-6">
                {t("svc.agents.capabilities.label")}
              </span>
              <h2 className="text-heading font-serif">{t("svc.agents.capabilities.title")}</h2>
              <div className="accent-bar mt-6" />
            </div>
              ...
              {capabilityKeys.map((key, i) => (
                ...
                >
                  ...
                  <h3 className="font-display text-xl tracking-tight mb-3">
                    {t(`svc.agents.capabilities.items.${key}.title`)}
                  </h3>
                  <p className="text-sm leading-relaxed text-muted-foreground">
                    {t(`svc.agents.capabilities.items.${key}.desc`)}
                  </p>
                </div>
              ))}
            </div>
